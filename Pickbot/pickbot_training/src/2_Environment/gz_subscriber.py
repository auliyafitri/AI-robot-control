...
Plan B:
 I suspect that if you don't want your users to have to use asyncio/trollius, 
 your best bet should run a trollius ioloop in a separate thread, and provide 
 an api that uses queues/etc to communicate with the trollius thread. 
 It'll be a bit clunky, but should work fine.

 You would need to call call_soon_threadsafe to do operations on the asyncio 
 thread from a different thread, and then your asyncio code could communicate 
 with the other threads via a queue or a concurrent.Future object (not the asyncio version, however!).

Plan C:
 Publish a ros topic with std.msg Bool telling whether collision is invalid (True)
        ...
        if descriptor.type == descriptor.TYPE_MESSAGE:
            ...
            while not(collision_2==''): #both collision_1 and collision_2 are assigned
                if not_to_be_ignored(): #not stand and box
                    ...
                    if not rospy.is_shutdown():
                        #print("collisions: %s and %s; done: %s at %s" % (collision_1, collision_2, _done, rospy.get_time()))
                        rospy.loginfo(_done)
                        pub.publish(_done)
                    '''
